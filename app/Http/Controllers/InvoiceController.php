...
class InvoiceController extends Controller
{
    ...
    public function InvoiceAdd(){
        ...
        $date = date('Y-m-d');
        return view('backend.invoice.add_invoice',compact('invoice_no','category','date','costomer'));
    } //End method
    ...
    public function Invoicestore(Request $request){
        ...
        $notification = array(
            'message' => 'Paid Amount is Greater than total price',
            'alert-type' => 'error'
        );
        return redirect()->back()->with($notification);
    ...
    public function InvoiceApprovalStore(Request $request, $id){
        foreach($request->selling_qty as $key => $val){
            ...
            if($product->quantity < $request->selling_qty[$key]){
                $notification = array(
                    'message' => 'Sorry your quantity value is low',
                    'alert-type' => 'error'
                );
                return redirect()->back()->with($notification);
            }
    ...
    public function DashboardInvoiceList(){
        $allData = Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','1')->get();
